<script lang="ts">
  ...
  async function generate() {
    error = '';
    try { result = (await generateClientConfig(token, { group, server_address: serverAddress, server_name: serverName || null, ca_certificate: caCertificate || null, services })).toml; }
...
{:else}
  ...
  <section class="section-block compact"><div class="section-heading"><div><h2>连接信息</h2><p>用于建立 QUIC 控制连接</p></div></div><div class="form-grid four"><label>访问分组<select bind:value={group}>{#each config.groups as item}<option value={item.name}>{item.name}</option>{/each}</select></label><label>服务器地址<input bind:value={serverAddress} /></label><label>TLS 服务器名称（可选）<input bind:value={serverName} /></label><label>CA 证书路径（可选）<input bind:value={caCertificate} /></label></div></section>
