...
脚本可交互选择 QUIC、Proxy 和 Web 模块，也支持无人值守安装。Web 未提供配置时会自动生成仅监听 `127.0.0.1:8080` 的安全初始配置，并一次性显示随机管理员密码；QUIC 和 Proxy 未提供有效配置时只安装 `*.example.toml`，不会阻止 Web 启动：
