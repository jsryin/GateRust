...
首次点击“获取配置”时，如果客户端配置目录没有可用的 `server.pem`，客户端会通过分组密钥双向证明验证服务端，并将证书保存为 `server.pem` 后重新建立受信任连接。整个获取过程最多持续 60 秒；超时或密钥错误时不会保存候选配置，也不会在后台继续获取。证书引导协议要求客户端与服务端同时升级到相同版本。
