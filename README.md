...
- 客户端首次启动自动创建用户配置，并通过 Electron 桌面界面管理连接和服务。
...
Release 提供 Linux x86_64/aarch64 AppImage、Windows x86_64 安装程序，以及 macOS Intel/Apple Silicon DMG。安装包已经包含 Rust 隧道后台，不需要另外安装 `client.toml`。Linux 下载后需要添加执行权限：
...
Windows 客户端文件名为 `gaterust-client-x64-win.exe`；macOS 安装镜像按架构命名为 `gaterust-client-x64-mac.dmg` 或 `gaterust-client-arm64-mac.dmg`。启动后直接在桌面窗口中填写服务器地址、分组密钥、TLS 和本地服务，不会跳转浏览器。首次启动生成的配置位于：
...
重复打开程序只会唤回已有窗口，不会启动重复连接。桌面程序支持通过 `--config` 使用指定配置文件，例如：

```bash
./gaterust-client-x64-linux.AppImage --config /path/to/client.toml
```
...
基于 [server.example.toml](config/server.example.toml) 创建服务端配置后启动。源码开发时先构建 Rust 后台，再启动 Electron 桌面端；客户端只需填写服务端分配的分组密钥和地址。使用公共 CA 证书时可省略 `name` 和 `ca_certificate`，TLS 名称会从服务器地址推导：
...
Electron 启动参数也支持 `--config config/client.toml` 使用指定路径，并会透传给 Rust 后台；文件不存在时会自动创建。首次启动时还会在配置文件旁生成 `client.toml.device-id`，用于在共享同一分组密钥时稳定区分设备。
