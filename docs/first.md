...
- **客户端**：首次启动自动创建用户配置，通过 Tauri 2 桌面界面设置服务器地址、分组密钥、TLS 和本地服务。
...
### 客户端
Tauri 2 桌面程序直接嵌入 Rust 客户端运行时，无需启动额外后台进程；认证配置保存后自动建立控制会话，隧道选择通过有界命令通道提交，并等待服务端确认后更新界面。

### 部署与安全（已补充 sh 新功能）
- **部署**：musl static binary + `deploy.sh`。
  - 支持一键**卸载某个模块**（干净卸载：停止对应服务、移除服务文件、清理模块相关配置/日志，按需保留主 binary）。
  - 支持一键**开启/关闭开机自启**（systemd 或 OpenRC）。
